...
def validate_event(json_path, scan_threshold, expected_index_version='v2'):
    ...
    if not isinstance(trigger_time, (int, float)):
        ...
        trigger_time = 0.0
    elif trigger_time < 1_000_000_000:
        errors.append('trigger_time is not a Unix timestamp')
    if not pre:
    ...
    for sample in samples:
        if not isinstance(sample.get('topic'), str):
            errors.append('one or more samples have no topic')
            break
        if not isinstance(sample.get('data'), dict):
            errors.append('one or more samples have invalid data')
            break
    ...
    for topic in required_topics:
        if topic not in observed_topics:
            errors.append(f'no sample contains topic {topic}')

    moving_samples = [
        sample for sample in pre
        if sample.get('topic') in ('/odom', '/cmd_vel')
        and abs((sample.get('data') or {}).get(
            'linear_speed',
            (sample.get('data') or {}).get('linear_x', 0.0),
        )) > 0.03
    ]
    ...
    observed_min = min(
        (
            (sample.get('data') or {}).get('front_min_distance', 999.0)
            for sample in samples
            if sample.get('topic') == '/scan'
        ),
        default=999.0,
    )
    ...
    if not any(
        ...
        and (sample.get('data') or {}).get('status') == 'EXECUTING'
        for sample in samples
    ):
    ...
    with open(agent_log_path, 'w', encoding='utf-8') as agent_log:
        try:
            agent_command = [
                ...
                '-p', f'run_id:={run_id}',
                '-p', 'event_type:=obstacle_near',
                '-p', f'robot_id:={options.robot_id}',
                '-p',
                f'static_index_version:={options.static_index_version}',
            ]
            backend_base_url = options.backend_base_url.strip()
            if backend_base_url:
                agent_command.extend([
                    '-p', f'backend_base_url:={backend_base_url}',
                ])
            ...
            warmup_wall_started = time.monotonic()
            wait_for_ros_duration(
                navigator,
                options.warmup_sec,
                wall_timeout_sec=max(60.0, options.warmup_sec * 10.0),
            )
            remaining_wall_warmup = (
                options.warmup_sec
                - (time.monotonic() - warmup_wall_started)
            )
            if remaining_wall_warmup > 0:
                time.sleep(remaining_wall_warmup)
